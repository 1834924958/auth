		...
		<div id="main" class="main">
			<div class="portlet box light-grey">
				<div class="portlet-title">
					<div class="caption"><i class="icon-globe"></i>文章管理</div>
					<div class="tools">
				...
				<div class="portlet-body">
					<div class="clearfix">
						<div class="btn-group">
							<a href="<?php echo U('Article/articlesEdit');?>">
						...
						<div class="btn-group">
							<button id="sample_editable_1_new" class="btn red ajax-post btn-warning" url="<?php echo U('Article/articlesStatus',array('type'=>'forbid'));?>" target-form="ids">
							禁用&nbsp;&nbsp;<i class="icon-lock"></i>
							</button>
						</div>
						<div class="btn-group">
							<button id="sample_editable_1_new" class="btn green ajax-post btn-info" url="<?php echo U('Article/articlesStatus',array('type'=>'resume'));?>" target-form="ids">
							启用&nbsp;&nbsp;<i class="icon-unlock"></i>
							</button>
						</div>
						<div class="btn-group">
							<button id="sample_editable_1_new" class="btn red ajax-post confirm btn-danger" url="<?php echo U('Article/articlesStatus',array('type'=>'del'));?>" target-form="ids">
						...
						<div class="btn-group pull-right">
							<form name="formSearch" id="formSearch" method="get" class="btn-group pull-right">
								<div class="btn-group pull-right" id="sample_1_filter">
									...
									<select style="width: 160px; height:34px;float: left; margin-right: 10px;" name="type" class="form-control">
										<option value=""
										<?php if(empty($_GET['type'])): ?>selected<?php endif; ?>
										>类型标题</option>
										<?php if(is_array($article_type)): $i = 0; $__LIST__ = $article_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$at): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"
										<?php if(($_GET['type']) == $key): ?>selected<?php endif; ?>
										><?php echo ($at); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
									...
									<a href="<?php echo U('Article/articles');?>">
					...
					<table class="table table-striped table-bordered table-hover" id="sample_1">
						...
						<tbody>
							<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
										...
										<td ><a href="<?php echo U('Article/articlesEdit?id='.$vo['id']);?>" class="btn mini purple"><i class="icon-edit"></i>&nbsp;编辑</a></td>
