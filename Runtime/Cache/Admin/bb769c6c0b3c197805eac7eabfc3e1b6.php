		...
		<div id="main" class="main">
			<div class="portlet box blue">
				<div class="portlet-title">
					<a href="<?php echo U('Article/adver');?>" style="color:white">
						<div class="caption"><i class="icon-reorder"></i>广告管理</div>
					</a>
					<div class="caption">
						<?php if(empty($data)): ?>&nbsp;&nbsp;广告添加
						<?php else: ?>
							&nbsp;&nbsp;广告编辑<?php endif; ?>
					</div>
					<div class="tools">
				...
				<div class="portlet-body form">
					<form action="<?php echo U('Article/adverEdit');?>" class="form-horizontal" method="POST" id="from">
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" name="name" value="<?php echo ($data["name"]); ?>" class="span6 m-wrap popovers" data-trigger="hover" data-content="*&nbsp;对图片的简单的介绍."  />
							</div>
						</div>
						<div class="control-group">
							<label class="control-label">图片链接：</label>
							<div class="controls">
								<input type="text" name="url" value="<?php echo ($data["url"]); ?>"  class="span6 m-wrap popovers" data-trigger="hover" data-content="*&nbsp;图片的Url地址，便于进行跳转."  />
							</div>
						</div>
						<div class="control-group">
							...
							<div class="controls">
								<div id="addpicContainer">
									...
									<?php if(!empty($data["img"])): ?><!--没有图片显示默认图片-->
										<img id="up_img" onclick="getElementById('inputfile').click()" style="cursor:pointer;max-width:500px;" title="点击添加图片" alt="点击添加图片" src="/Upload/adver/<?php echo ($data["img"]); ?>">
										<?php else: ?>
										<!--没有图片显示默认图片-->
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text"  name="endtime" value="<?php echo (addtime($data['endtime'])); ?>" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" class="span6 m-wrap popovers" data-trigger="hover" data-content="*&nbsp;在表中进行对本条数据编辑的时间."  />
							</div>
						</div>
						<div class="control-group">
							<label class="control-label">排序：</label>
							<div class="controls">
								<input type="text"  name="sort" value="<?php echo ($data["sort"]); ?>"  class="span6 m-wrap popovers" data-trigger="hover" data-content="*&nbsp;不同的值在前台中代表在前台中不同的位置."  />
							</div>
						</div>
						<div class="control-group">
						...
						<div class="form-actions">
							...
							<a class="btn-return" href="<?php echo ($_SERVER['HTTP_REFERER']); ?>">
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#inputfile').change(function(){
			...
			//执行发送数据
			$.ajax({
				url:"<?php echo U('Article/adverImage');?>",
				type:'POST',
				data:data,
				cache:false,
				contentType:false,
				...
				success:function(data){
					if(data){
						$('#up_img').attr("src","/Upload/adver/" + data);
						$('#img').val(data);
						$('#up_img').show();
					}
				},
